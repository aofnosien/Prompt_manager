...
over three finger(fingers excluding thumb):2,
fused anatomy,
bad anatomy(body),
bad anatomy(hand),
bad anatomy(finger),
over four fingers(finger):2,
bad anatomy(arms),
over two arms(body),
bad anatomy(leg),
over two legs(body),
interrupted(body, arm, leg, finger),
bad anatomy(arm),
{
bad detail(finger),
bad anatomy(fingers),
...
multiful(arms),
multiful(legs),
bad detailed background,
unclear architectural outline,
...
";
        ...
        public Dictionary<string, (Func<string>, Action<StringReader>)> option_quests = new();
        ...
        public void LoadOptions()
        {
            ...
            {
                var fulltext = "";
                using (StreamReader sr_ = new StreamReader(optionFilePath))
                    fulltext = sr_.ReadToEnd();
